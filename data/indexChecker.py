...
if __name__ == "__main__":
	...
	for line in fin:
		...
		pmid, sentence, event_id, event_type, confidence_min, Estr, EoffBeg, EoffEnd, theme_event_id, E2str, E2offBeg, E2offEnd, theme_ggp_id, Gstr, GoffBeg, GoffEnd = items[:16]
		cancers = items[16:]
		...
		if errors:
			...
			indexes = list()
			for error in errors:
				indexes.append([int(error[1]) - m.start() for m in re.finditer(error[0], sentence)])
		...
		if args.mode == "fix": print >> fout, '\t'.join([pmid, sentence, event_id, event_type, confidence_min, Estr, str(EoffBeg), str(EoffEnd), theme_event_id, E2str, str(E2offBeg), str(E2offEnd), theme_ggp_id, Gstr, str(GoffBeg), str(GoffEnd)]+cancers)
